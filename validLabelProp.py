...
for i, (images, labels) in enumerate(valloader):
    ...
    if torch.cuda.is_available():
        ...
        inputs = Variable(torch.cuda.FloatTensor(currBSize, chCnt, H, W))
        outputs = Variable(torch.cuda.LongTensor(currBSize, H, W))
        predClass = Variable(torch.cuda.LongTensor(currBSize, H, W))
    else:
        images = Variable(images)
        labels = Variable(labels)
        inputs = Variable(torch.FloatTensor(currBSize, chCnt, H, W))
        outputs = Variable(torch.LongTensor(currBSize, H, W))
        predClass = Variable(torch.LongTensor(currBSize, H, W))
    ...
    running_acc += torch.sum(predClass.data == outputs.data) * outSize * 100
...
meanIoU = torch.sum(IoU/imgCnt)/numClass*100*2
